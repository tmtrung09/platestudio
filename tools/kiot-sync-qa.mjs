...
try {
  ...
  assert.match(workerSource, /else \{tab=await chrome\.tabs\.update\(tab\.id,\{active:true\}\);await chrome\.tabs\.reload\(tab\.id\);\}/, 'Mỗi ngày bù phải reload lại trang Báo cáo để cô lập state lỗi của KiotViet');
  ...
  console.log(JSON.stringify({ summary: { passed: 23, failed: 0 }, range: 'sequential custom-day queue with reload isolation, version guard, and date verification' }, null, 2));
} finally {
  child.kill();
