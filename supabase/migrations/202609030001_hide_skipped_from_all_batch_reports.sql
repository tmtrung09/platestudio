-- “Tất cả” là danh sách báo cáo đang sử dụng. Ảnh được phân loại Bỏ qua chỉ
-- được trả về khi người dùng chọn bộ lọc Bỏ qua, kể cả khi phân trang ở server.
...
as $$
  with base as (
    ...
    select * from base
    where (p_status = 'all' and coalesce(data->>'status', 'pending') <> 'skipped')
...
as $$
  with filtered as (
    ...
    where br.workspace_id = p_workspace_id
      and (coalesce(btrim(p_query), '') = '' or lower(br.data::text) like '%' || lower(btrim(p_query)) || '%')
      and (
        (p_status = 'all' and coalesce(br.data->>'status', 'pending') <> 'skipped')
